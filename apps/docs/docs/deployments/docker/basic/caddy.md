...
Caddy is a small, secure, and zero‑config web server that provides automatic HTTPS. It makes hosting chithi instances simple and reliable.
...
Here is a ready-to-use `docker-compose.yml` that runs Caddy as the reverse proxy for `frontend` and `backend` services:
...
Create a `Caddyfile` next to your `docker-compose.yml` with this contents:
...
<your_domain> {
  ...
  # Optional: set an email for automatic TLS
  tls you@example.com
}
```
